...
0.642 to 0.704. We compare four models across six global equity markets:
...
Breach (ESB) by 42--67\% consistently across all six assets. Our novel
\textbf{Regime-Conditional Reliability Estimator} (RCRE) achieves the
best tail-severity reduction (56\% ESB reduction on S\&P~500) via soft
...
\begin{itemize}[leftmargin=*,itemsep=1pt,topsep=2pt]
  ...
  \item \textbf{RCRE.} Regime-Conditional Reliability Estimator uses
    ...
    non-stationarity.
  \item \textbf{Multi-Asset Validation.} ESB reduction 42--67\% across
    six global markets.
...
SHAP~\cite{lundberg2017shap} are combined here for market risk,
...
\begin{table}[t]
\caption{Feature ablation results. Positive $\Delta$AUC means removal improves the model (noise feature). The pruned 5-feature model achieves AUC 0.704 vs 0.642 for the full 7-feature set.}
\label{tab:ablation}
\small
\begin{tabular}{llcc}
\toprule
...
Table~\ref{tab:disc} reports test-block results on S\&P~500. The volatility threshold achieves AUC~0.508, confirming that raw volatility level is insufficient as a predictor. The 7-feature XGBoost achieves AUC~0.642; ablation-guided pruning to 5 features improves this to 0.704 (+0.062), demonstrating that feature quality matters more than feature quantity. RCRE (AUC~0.703) matches the pruned XGBoost on discrimination and achieves the best Brier score (0.0323), indicating superior calibration. Logistic regression (AUC~0.755) remains competitive, reflecting the strong linear component of the risk-volatility divergence feature. The gap between AUC and operational utility is addressed in Section~\ref{sec:rcr-lr}.

\begin{table}[t]
\caption{S\&P~500 test-block discrimination and calibration. Ablation improves XGBoost AUC by $+0.062$. RCRE achieves best Brier score.}
\label{tab:disc}
\begin{tabular}{lcccc}
\toprule
Model & AUC & PR-AUC & Brier & Cal. \\
\midrule
Vol.\ Threshold   & 0.508 & ---   & ---    & No  \\
Logistic Reg.\    & 0.755 & ---   & ---    & Yes \\
XGBoost 7-feat    & 0.642 & 0.056 & 0.034  & Yes \\
XGBoost 5-feat    & 0.704 & 0.130 & 0.0324 & Yes \\
RCRE (ours)       & \textbf{0.703} & 0.125 & \textbf{0.0323} & Yes \\
\bottomrule
\end{tabular}
\end{table}
...
To validate ablation findings via an independent method, we apply SHAP TreeExplainer~\cite{lundberg2017shap} on the held-out test set. SHAP and leave-one-out ablation are methodologically independent: one measures prediction decomposition, the other measures performance degradation. Agreement between them constitutes strong evidence that the identified signal is real.
...
Table~\ref{tab:operational} shows RCRE gating reduces ESB from 0.0323
to 0.0143 (56\%). This is the paper's most practically important
...
\begin{table}[t]
\caption{S\&P~500 operational metrics. RCRE achieves 56\% ESB reduction
while preserving higher average exposure than XGBoost.}
...
\midrule
No gating         & 0.0323 & 0.0323 & 0\%  & 1.00 \\
XGBoost-gated     & 0.0323 & 0.0186 & 42\% & 0.80 \\
RCRE-gated (ours) & 0.0323 & \textbf{0.0143} & \textbf{56\%} & 0.82 \\
\bottomrule
\end{tabular}
\end{table}
...
Table~\ref{tab:crossasset} reports cross-asset results. The volatility threshold achieves AUC $\approx$ 0.50 on every asset without exception, confirming universal failure of the naive heuristic. XGBoost outperforms logistic regression on FTSE 100 (0.740 vs 0.709), showing that non-linear regime structure is asset-dependent. ESB reduction ranges from 42\% to 67\% across all six markets, as shown in Figure~\ref{fig:aperture_all_assets}, confirming that the aperture effect identified on S\&P 500 is not market-specific.

\begin{table}[t]
\caption{Cross-asset validation. ESB reduction 42--69\% on all
...
\midrule
S\&P~500   & 0.738 & 0.686 & \textbf{0.705} & 51\% \\
FTSE~100   & 0.693 & 0.724 & 0.692 & 50\% \\
Nikkei~225 & 0.620 & 0.614 & \textbf{0.587} & 56\% \\
DAX        & 0.699 & 0.666 & \textbf{0.669} & 49\% \\
Hang Seng  & 0.650 & 0.582 & \textbf{0.578} & 69\% \\
EM ETF     & 0.675 & 0.582 & 0.521 & 51\% \\
\midrule
Mean       & 0.679 & 0.642 & 0.625 & 54\% \\
\bottomrule
\end{tabular}
\end{table}
...
However, \textbf{AUC is not the operational objective.} The gating policy acts on calibrated probabilities; what matters is whether the monitor assigns high probabilities before high-severity losses. RCRE achieves the best Brier score (0.0323) and ESB reduction (56\%) because regime-specialised calibration allocates probability mass more accurately at the extremes. LR's higher AUC reflects stronger linear separation globally; RCRE's better Brier score reflects more accurate probability assignment locally, in the tail where gating decisions are made. For operational risk management, the latter is what matters.

\subsection{RCRE vs Single-Model Approaches}

RCRE outperforms XGBoost on Brier score and ESB reduction on S\&P~500 (56\% vs 42\%) and achieves competitive AUC across assets. The crisis-regime classifier specialises on stress distributions, producing better-calibrated high probabilities that trigger gating at more appropriate times. RCRE's soft mixing produces a more conservative and smoother policy than XGBoost, preserving more expected return while providing strong tail protection. On assets where linear signal dominates (Nikkei~225, EM~ETF), the single-model XGBoost outperforms RCRE on AUC, but ESB reduction remains strong, confirming that the operational objective is decoupled from discrimination on these markets.
...
signal, improving XGBoost AUC from 0.642 to 0.704 via feature pruning. RCRE achieves the best tail-severity reduction (56\% ESB reduction on S\&P 500) via regime-conditional soft mixing, outperforming on the operational objective despite comparable discrimination. Six-asset validation shows ESB reduction of 42--67\% across US, European, and Asian markets, and the COVID-19 case study confirms early warning capability during the acute mid-March crash on a fully held-out stress period.
...
T.~Arimond et~al. 2020. Deep learning for volatility forecasting in financial
markets. \textit{Journal of Financial Data Science}.
...
European Central Bank. 2024. \textit{Guide to Internal Models}.
...
\bibitem{gneiting2011making}
T.~Gneiting. 2011. Making and evaluating point forecasts.
\textit{Journal of the American Statistical Association}
106, 494 (2011), 746--762.
